...
#define MAX_CORONATION_TIME 60 * 60 * 24 * 1
#define CLAIM_MULTIPLIER 1.35
#define COMMISSION_PERCENTAGE_POINTS 0.05

const account_name contract_fee_account = N(feeofeosking);

class eoskingone : public eosio::contract
{
  public:
    eoskingone(account_name self)
        : contract(self),
          claims(self, self)
    {
    }
    ...
    // the first argument of multi_index must be the name of the table in the ABI!
