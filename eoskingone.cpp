...
void eoskingone::onTransfer(const currency::transfer &transfer)
{
    ...
    {
        asset amount = asset{(int64_t)((1 - COMMISSION_PERCENTAGE_POINTS) * claimPrice), transfer.quantity.symbol};
        asset fee = asset{(int64_t)(COMMISSION_PERCENTAGE_POINTS * claimPrice), transfer.quantity.symbol};
        action{
            permission_level{_self, N(active)},
            N(eosio.token),
            N(transfer),
            currency::transfer{
                .from = _self, .to = latestClaimRecord.claim.name, .quantity = amount, .memo = "You were dethroned! Here's your profit. - eoskingone"}}
            .send();
        action{
                permission_level{_self, N(active)},
                N(eosio.token),
                N(transfer),
                currency::transfer{
                        .from = _self, .to = contract_fee_account, .quantity = fee, .memo = "fee of eoskingone"}}
                .send();
    }
}
...
void eoskingone::init(account_name name)
{
    require_auth(_self);
...
void eoskingone::apply(account_name contract, account_name act)
{
    ...
    switch (act)
    {
        ...
        EOSIO_API(eoskingone, (init)(end))
    };
}

// EOSIO_ABI only works for contract == this conract
// EOSIO_ABI(eoskingone, (transfer))

extern "C"
{
    [[noreturn]] void apply(uint64_t receiver, uint64_t code, uint64_t action) {
        eoskingone king(receiver);
